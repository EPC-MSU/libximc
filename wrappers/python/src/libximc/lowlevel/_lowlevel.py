...
def _load_lib():
    ...
    if os_kind == "windows":
        ...
        if 8 * struct.calcsize("P") == 32:
            ...
            path = os.path.join(cwd, r"..\library-files\win32")
        else:
            libs = ("bindy.dll", "xiwrapper.dll", "libximc.dll")
            path = os.path.join(cwd, r"..\library-files\win64")
    elif os_kind == "linux":
        method = CDLL
        cpu_kind = machine().lower()
        libs = ("libbindy.so", "libxiwrapper.so", "libximc.so")
        if cpu_kind == "arm":
            cpu_path = os.path.join(cwd, "../library-files/debian-armhf")
        elif cpu_kind == "i386":
            cpu_path = os.path.join(cwd, "../library-files/debian-i386")
        else:
            cpu_path = os.path.join(cwd, "../library-files/debian-amd64")
        path = cpu_path
    else:
